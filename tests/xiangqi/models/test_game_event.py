...
def test_open_draw_offers(game, draw_events):
    assert set(GameEvent.open_draw_offers.all()) == set(draw_events)


@pytest.fixture
def draw_events_with_rejections(game, game_event_factory):
    return [
        game_event_factory(game=game, name="offered_draw"),
...
@pytest.mark.django_db
def test_open_draw_offers_with_rejections(game, draw_events_with_rejections):
    assert set(GameEvent.open_draw_offers.all()) == set(
        [draw_events_with_rejections[-1]]
    )
